...
### Fixed
- **Capture interface save button**: Fixed "method not allowed" error — was using POST instead of PUT for config endpoint

## [1.0.40] - 2026-02-20

### Added
- **WiFi capture interface selector**: Dropdown on the Packet Capture tab lists all WiFi interfaces with driver name and monitor mode support indicator
- **`WIFI_CAPTURE_IFACE` config option**: Set to `auto` (default) or a specific interface like `wlan1` for the USB adapter
- **`/api/wifi/interfaces` endpoint**: Lists all wireless interfaces with driver and monitor mode capability
- Capture status now shows which interface is active
...
### Fixed
- **Monitor mode enabling**: More robust process — kills wpa_supplicant per-interface, waits for release, adds iwconfig fallback, verifies mode is actually set
- **Better error messages**: Capture start failures now show the actual error (e.g., driver doesn't support monitor mode) instead of generic "failed"
- **Capture timeouts**: Start increased to 45s, restart to 60s to accommodate monitor mode setup
